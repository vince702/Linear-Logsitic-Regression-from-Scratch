...
def getBeta(train_x, train_y):   
    ...
    A = np.linalg.inv(train_x.transpose().dot(train_x) )
    B = train_x.transpose().dot(train_y)
    beta = A.dot(B)
    return beta
...
def getBetaStochasticGradient(train_x, train_y, lr):
    ...
    for iter in range(epoch):
        ...
        for i in range(n):
            idx = indices[i]
            ########## Please Fill Missing Lines Here ##########
            loss = train_y[idx] -train_x[idx].transpose().dot(beta)
            change = train_x[idx].dot(loss)
            beta += lr * change
