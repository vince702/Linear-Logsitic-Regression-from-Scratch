...
def compute_avglogL(X,y,beta):
    ...
    loglikelihood = 0
    for i in range (0,n):
        xTdotBeta = X[i].transpose().dot(beta)
        loglikelihood += y[i]*(xTdotBeta) - np.log(1 + np.exp(xTdotBeta))
    avglogL = loglikelihood/n
    return avglogL
...
def getBeta_BatchGradient(train_x, train_y, lr, num_iter, verbose):
    ...
    for iter in range(0, num_iter):
        ...
        gradient = train_x.transpose().dot( sigmoid(train_x.dot(beta)) - train_y)
        beta = beta - gradient
...
def getBeta_Newton(train_x, train_y, num_iter, verbose):
    ...
    ########## Please Fill Missing Lines Here ##########
        ...
        sigm = sigmoid(train_x.dot(beta))
        
 
        Hessian = np.matmul(train_x.transpose(),(np.diag(sigm*(1-sigm))) )
        Hessian = np.matmul(Hessian, train_x)
        
        gradient = train_x.transpose().dot( sigmoid(train_x.dot(beta)) - train_y)
        beta = beta - np.linalg.inv(Hessian).dot(gradient)
